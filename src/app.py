...
@app.route("/add_contact", methods=["POST"]) #lleva la informacion desde el formulario
def add_contact():
    if request.method == "POST":
        ...
        return redirect (url_for("index"))#redirigir
...
@app.route("/update/<id>",methods=["POST"]) #envia la informacion
def update_contact(id):
    if request.method == "POST":
        ...
        cur.execute("""
            ...
            email = %s
            where id = %s
        """,(nom,tel,email,id) ) #realizando el cambio y pasando los datos
        mysql.connection.commit()
        flash("Contacto Actualizado Correctamente")
        return redirect (url_for("index")) #redirigia a la nueva pagina
